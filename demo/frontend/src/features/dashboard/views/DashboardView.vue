<template>
  <MainLayout>
    ...
    <div v-else>
      ...
      <div class="content-row operations-row">
        <div class="panel">
          <div class="panel-head"><h3>Agent 运行概览</h3></div>
          <div class="panel-body operation-list">
            <div class="operation-item"><span>会话总数</span><b>{{ traceSummary.total_sessions || 0 }}</b></div>
            <div class="operation-item"><span>已完成会话</span><b>{{ traceSummary.completed_sessions || 0 }}</b></div>
            <div class="operation-item"><span>发生工具调用</span><b>{{ traceSummary.sessions_with_tool_calls || 0 }}</b></div>
            <div v-for="agent in agentMetrics.agents || []" :key="agent.agent" class="operation-item muted-item">
              <span>{{ agent.agent }}</span><b>{{ agent.trace_count || 0 }}</b>
            </div>
          </div>
        </div>
        <div class="panel">
          <div class="panel-head"><h3>招聘漏斗与交付</h3></div>
          <div class="panel-body operation-list">
            <div v-for="(value, stage) in funnel" :key="`f-${stage}`" class="operation-item"><span>{{ stage }}</span><b>{{ value }}</b></div>
            <div v-for="(value, stage) in outreachKanban" :key="`k-${stage}`" class="operation-item muted-item"><span>{{ stage }}</span><b>{{ value }}</b></div>
            <div class="operation-item"><span>面试报告</span><b>{{ reportOverview.reports || 0 }}</b></div>
          </div>
        </div>
      </div>
    </div>
...
import {
  dashboardStats, dashboardTracesSummary, dashboardFunnel,
  dashboardOutreachKanban, dashboardReportOverview, dashboardAgentMetrics,
} from '@/api'
...
async function loadStats() {
  loading.value = true
  try {
    const [statsResult, traceResult, funnelResult, kanbanResult, reportResult, metricsResult] = await Promise.allSettled([
      dashboardStats(), dashboardTracesSummary(), dashboardFunnel(), dashboardOutreachKanban(),
      dashboardReportOverview(), dashboardAgentMetrics(),
    ])
    stats.value = statsResult.status === 'fulfilled' ? statsResult.value || {} : {}
    traceSummary.value = traceResult.status === 'fulfilled' ? traceResult.value || {} : {}
    funnel.value = funnelResult.status === 'fulfilled' ? funnelResult.value || {} : {}
    outreachKanban.value = kanbanResult.status === 'fulfilled' ? kanbanResult.value || {} : {}
    reportOverview.value = reportResult.status === 'fulfilled' ? reportResult.value || {} : {}
    agentMetrics.value = metricsResult.status === 'fulfilled' ? metricsResult.value || {} : {}
  } catch {
    // 静默失败，显示默认值
  } finally {
...
<style scoped>
.operations-row { margin-top: var(--space-5); }
.operation-list { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; }
.operation-item { display: flex; justify-content: space-between; gap: 12px; padding: 10px 12px; border-radius: 8px; background: var(--surface-warm); color: var(--fg-2); font-size: 13px; }
.operation-item b { color: var(--fg); font-variant-numeric: tabular-nums; }
.muted-item { background: transparent; border: 1px solid var(--border); }
@media (max-width: 720px) { .operation-list { grid-template-columns: 1fr; } }
</style>
